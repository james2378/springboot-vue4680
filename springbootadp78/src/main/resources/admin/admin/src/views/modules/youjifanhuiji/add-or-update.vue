<template>
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"45%","margin":"0 0 20px 0","display":"inline-block"}' class="select" v-if="type!='info'" label="产品名称" prop="chanpinmingcheng">
					<el-select :disabled="ro.chanpinmingcheng" @change="chanpinmingchengChange" v-model="ruleForm.chanpinmingcheng" placeholder="请选择产品名称">
						<el-option
							v-for="(item,index) in chanpinmingchengOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"45%","margin":"0 0 20px 0","display":"inline-block"}' class="input" v-else-if="ruleForm.chanpinmingcheng" label="产品名称" prop="chanpinmingcheng">
				...
				<el-form-item :style='{"width":"45%","margin":"0 0 20px 0","display":"inline-block"}' class="textarea" v-if="type!='info'" label="故障描述" prop="guzhangmiaoshu">
					<el-input
				...
				<el-form-item :style='{"width":"45%","margin":"0 0 20px 0","display":"inline-block"}' v-else-if="ruleForm.guzhangmiaoshu" label="故障描述" prop="guzhangmiaoshu">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.guzhangmiaoshu}}</span>
				</el-form-item>
		...
		init(id,type) {
			...
			this.$http({
				...
				if (data && data.code === 0) {
					...
					if(json.dianhuahaoma!=''&&json.dianhuahaoma && this.$storage.get("role")!="管理员"){
			...
			// 下二随
			chanpinmingchengChange () {
				this.$http({
					url: `follow/chanpinxinxi/chanpinmingcheng?columnValue=`+ this.ruleForm.chanpinmingcheng,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						if(data.data.chanpinfenlei){
							this.ruleForm.chanpinfenlei = data.data.chanpinfenlei
						}
					} else {
						this.$message.error(data.msg);
					}
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `youjifanhuiji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.youjifanhuijiCrossAddOrUpdateFlag = false;
						 this.parent.search();
