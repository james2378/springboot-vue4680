...
export default {
	data() {
		...
		return {
			...
			rules: {
				shoulijifenlei: [
					{ required: true, message: '受理机分类不能为空', trigger: 'blur' },
				],
			}
		};
	},
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shoulijifenlei/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shoulijifenleiCrossAddOrUpdateFlag = false;
						 this.parent.search();
